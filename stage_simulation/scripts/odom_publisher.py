...
class OdometryPublisher:
  def __init__(self):
    self.pub_odom = rospy.Publisher(
      "odom_check", Odometry, queue_size=10)
    self.sub_twist = rospy.Subscriber(
      "robot_vel", Twist, self.twistCallback)
    ...
    quat_init = ts.quaternion_from_euler(0, 0, 0)
  ...
  def twistCallback(self, data):
    dt = (rospy.Time.now() - self.prev_time).nsecs / 1.e9
    self.prev_time = rospy.Time.now()
    ...
    (r, p, theta) = ts.euler_from_quaternion(
    ...
    if abs(w) > 1.e-3:
      self.odom.pose.pose.position.x += v / w * \
        (sin(theta_new) - sin(theta))
      self.odom.pose.pose.position.y -= v / w * \
        (cos(theta_new) - cos(theta))
    else:
      dl = v * dt
    ...
    quat_new = ts.quaternion_from_euler(0, 0, theta_new)
